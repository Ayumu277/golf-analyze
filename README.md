...
- 🎥 **動画アップロード**: ゴルフスイング動画をドラッグ&ドロップ（最大100MB）
...
1. **動画アップロード**
   ...
   - MP4, MOV, AVI などの動画ファイルに対応（最大100MB）
...
### 推奨事項
- Vercel Pro planの使用を推奨
- ファイルサイズ50MB以下を推奨（解析速度向上）
- 短い動画（30秒〜2分）を推奨
...
2. **「動画ファイルのサイズが100MBを超えています」エラー**
   - Vercelの制限により100MB以下のファイルを使用
   - 動画圧縮ツールを使用してファイルサイズを削減

3. **解析が遅い・タイムアウトする**
   - 大きなファイル（50MB以上）は解析に数分かかる場合があります
   - ネットワーク接続を確認
   - ファイルサイズを可能な限り小さくする（推奨: 30MB以下）
...
### 推奨事項
- **ファイルサイズ**: 30MB以下を推奨（解析速度・Vercel制限考慮）
- **動画時間**: 30秒〜2分程度を推奨
- **解像度**: 1080p以下を推奨
